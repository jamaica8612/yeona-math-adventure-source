...
namespace Antura.Core.Services.OnlineAnalytics
{
    public class Analytics : MonoBehaviour
    {
        ...
        private static bool IsDedicatedYeonaMathPackage =>
            string.Equals(
                Application.identifier,
                YeonaMathPackageIdentifier,
        ...
        private bool AnalyticsEnabled => !IsDedicatedYeonaMathPackage &&
            AppManager.I.AppEdition.OnlineAnalyticsEnabled && AppManager.I.AppSettingsManager.Settings.ShareAnalyticsEnabled;
        ...
        async void Awake()
        {
            if (IsDedicatedYeonaMathPackage)
            {
                // The child-facing Math APK is strictly local-first: do not initialize Unity
                // Services and make every analytics event path remain disabled.
                enabled = false;
                return;
            }
