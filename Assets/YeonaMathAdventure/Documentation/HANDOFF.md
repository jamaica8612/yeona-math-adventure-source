...
### 2026-07-15 추가: 재실행 시 전체 텍스트 소실 버그 수정

실기기 영상에서 앱을 껐다 다시 켜면 모든 라벨이 빈 채로 나오는 증상 확인(간헐).
원인: `KoreanFontProvider.Resolve()`가 빌드 때 구워 둔 폰트 에셋을 **매 실행마다 Dynamic
모드로 되돌려서**, FontEngine이 소스 폰트를 다시 여는 취약 경로(과거 기기 회귀와 동일
계열)를 항상 타고 있었음. 수정:

- 런타임: 구운 에셋은 그대로 사용 (`MathJourneyContracts.cs`의 Dynamic 강제 제거)
- 빌드: 글리프를 전부 구운 뒤 `AtlasPopulationMode.Static`으로 고정
  (`YeonaMathAndroidBuild.CreatePersistentPretendardFontAsset`)
- 회귀 테스트를 Static 기대값으로 갱신 + 신규 문자열 글리프('랗','빛') 단언 추가
- 주의: 새 한국어 문자열을 코드에 추가하면 반드시 `Yeona Math > Build`(내부에서
  SetupProject 실행)로 빌드해야 아틀라스에 반영된다. 힌트 버튼 글자색 대비도 수정
  (연보라 배경 + 흰 글자 → 진남색 글자).
- 검증 방법: 빌드 후 앱을 **3회 이상 완전 종료→재실행**하며 시작 화면 텍스트 확인.
